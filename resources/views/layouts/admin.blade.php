                ...
                <div class="section-content-right">
                    ...
                    <div class="main-content">
                        ...
                        <div class="bottom-page">
                            <div class="body-text">Copyright © 2024 Napoleon Cake</div>
                        </div>
                    </div>
    ...
    <script>
        $(function(){
            $("#search-input").on("keyup", function() {
                ...
                // Lakukan pencarian jika query lebih dari 2 karakter
                if (searchQuery.length > 2) {
                    $.ajax({
                        type: "GET",
                        url: "{{ route('admin.search') }}", // Sesuaikan dengan route admin
                        data: { query: searchQuery },
                        dataType: 'json',
                        success: function(data) {
                            $("#box-content-search").html(''); // Kosongkan konten sebelumnya

                            if (data.length === 0) {
                                $("#box-content-search").append('<li>Tidak ada produk ditemukan.</li>');
                            }

                            $.each(data, function(index, item) {
                                var productUrl = "{{ route('admin.product.edit', ['id' => 'product_id']) }}";
                                productUrl = productUrl.replace('product_id', item.id);

                                var imageUrl = item.image
                                    ? `{{ asset('uploads/products/thumbnails') }}/${item.image}`
                                    : '{{ asset('uploads/products/thumbnails/default.jpg') }}';
                        ...
                        error: function(xhr) {
                            $("#box-content-search").html('<li>Terjadi kesalahan saat memuat data.</li>');
                        }
                    });
                } else {
                    $("#box-content-search").html(''); // Kosongkan hasil jika query kurang dari 2 karakter
                }
            });
        });
    </script>
