        ...
        <section class="product-single container">
            <div class="row">
                ...
                <div class="col-lg-5">
                    ...
                    <!-- Menambahkan pengecekan jika produk sold out -->
                    @if(Cart::instance('cart')->content()->where('id', $product->id)->count() > 0)
    <a href="{{ route('cart.index') }}" class="btn btn-warning mb-3">Go To Cart</a>
@else
    @if ($product->quantity <= 0 || \Carbon\Carbon::now()->greaterThan(\Carbon\Carbon::parse($product->expiry_date)))
        <!-- Jika sold out atau expired -->
        <button type="button" class="btn btn-secondary btn-addtocart" disabled>Sold Out / Expired</button>
    @else
                                ...
                                <div class="pc__info position-relative">
                                    ...
                                    <div class="product-card__price d-flex">
                                        <span class="money price">
                                            @if ($product->sale_price)
                                                <s>{{ $product->formatted_regular_price }}</s> {{ $rproduct->formatted_sale_price }}
                                            @else
                                                {{ $product->formatted_regular_price }}
                                            @endif
                                        </span>
